...
| --- | --- | --- | --- |
| **Supabase Auth** | Email OTP-code sign-in, session refresh, sign-out, admin user deletion | `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` (sign-in); `SUPABASE_SERVICE_ROLE_KEY` (admin deletion); custom SMTP (Brevo) now configured in the Supabase dashboard for deliverability | Sign-in/session: **implemented, not yet re-verified against the provider after this change**. A real user's repeated magic-link failures (`"One-time token not found"` in the auth logs, consistent with an email client's link-scanner consuming the single-use link) showed the click-through flow itself was the problem, not just this app's error handling (`docs/decisions/0007-magic-link-error-surfacing.md` fixed the latter; `docs/decisions/0009-otp-code-sign-in.md` addresses the former by switching to a typed 6-digit code). **Still needs a manual step this environment can't perform**: the Supabase dashboard's "Magic Link" email template must be edited to show `{{ .Token }}` instead of a clickable link before the fix is complete — see `docs/roadmap.md`. Admin user deletion remains **implemented**, not yet verified — no service-role key in this environment. |
...
| **CSV/vCard contact import** (`packages/domain/src/contact-import.ts`, `apps/web/src/server/import`) | Parse CSV/vCard, guess+confirm field mapping, flag duplicates, create people (+ birthdays), undo an import | none | **Verified against the provider** — seeded the exact insert shapes `confirmImport`/`undoImport` perform (people + a birthday date, then a full undo) directly against the live Supabase project as the `authenticated` role, and confirmed a second user can neither see nor delete the first user's imported rows. All seeded rows removed afterward. One-way only by design — this never updates an existing person, so there is no "silently overwrite newer data" risk to test for. |
| **Device-contact picker** (`apps/web/src/components/browser-contact-picker.ts`) | Import contacts via the browser's native picker | none | **Implemented**, against the real W3C Contact Picker API (not a mock) — but **not verified against the provider**: that API only exists on Chrome for Android, and no mobile Chrome browser is available in this environment to exercise it. `isAvailable()` feature-detects and hides the option everywhere else. |
| **Notification preference centre** (`apps/web/src/server/profile`) | Edit `preferredReminderChannel`/`defaultReminderOffsets` after onboarding | none | **Verified against the provider** — updated a seeded profile's preferences as its owning user, confirmed a second user's identical update attempt against the first user's row affected zero rows (RLS), then cleaned up. |
| **Direct/scheduled send providers** | none yet | — | **Not started.** Genuinely blocked, not merely deferred: needs a real provider account (WhatsApp Business API, Twilio, etc.) to build an adapter against at all; gated on Stage 3's approval-binding decision (`docs/state-transitions.md`) regardless. |
| **Cloud contact providers** (Google/Apple/Microsoft) | none yet | — | **Not started.** Genuinely blocked: needs real OAuth app registrations (client id/secret, approved redirect URIs) with each provider, which this environment has no way to create — see `docs/roadmap.md`'s "Stage 4 remaining work." |
